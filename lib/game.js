(function () {
  ...
  Game.prototype.setEggsRemaining = function () {
    ...
    this.ctx.font = "40px Oswald";
    this.ctx.fillStyle = "blue";
    this.ctx.fillText(this.eggsCollected + "/" + this.totalEggs, 910, 55);
  };
  ...
  Game.prototype.setLivesImage = function () {
    ...
    for (var i = 0; i < this.chicken.lives; i++) {
      ...
      this.ctx.drawImage(heartIMG, x, 43);
      x += 30;
    }
  };

  Game.prototype.drawLevel = function () {
    this.ctx.font = "bold 32px Oswald";
    this.ctx.fillStyle = "blue";
    this.ctx.fillText("Level " + this.level, 30, 35);
  };
  ...
  Game.prototype.drawTimer = function () {
    ...
    if (minutes > 0) {
      ...
      if (this.seconds % 60 > 9) {
        this.ctx.fillText(minutes + ":" + remainder, that.dim_x/2 - 35, 55);
      }
      else {
        this.ctx.fillText(minutes + ":0" + remainder, that.dim_x/2 - 35, 55);
      }
    }
    else {
      if (this.seconds > 9) {
        this.ctx.fillText(minutes + ":" + this.seconds, that.dim_x/2 - 35, 55);
      }
      else {
        this.ctx.fillText(minutes + ":0" + this.seconds, that.dim_x/2 - 35, 55);
      }
    }
  };
  ...
  Game.prototype.collectEgg = function (egg) {
    ...
    var takeEggSound = new Audio("./audio/take_egg.mp3");
    takeEggSound.play();
    this.removeEgg(egg);
  };
  ...
  Game.prototype.upLevel = function () {
    ...
    if (this.eggsCollected === this.totalEggs) {
      this.level += 1;
      var levelWonSound = new Audio("./audio/cheer.mp3");
      levelWonSound.play();
      ...
      if (this.level === 2) {
        ...
        this.totalEggs = 2 + ((this.level - 2) * 2);
      }
  ...
  Game.prototype.collectLife = function (heart) {
    ...
    var takeLifeSound = new Audio("./audio/take_egg.mp3");
    takeLifeSound.play();
  ...
  Game.prototype.gameOver = function () {
    ...
    if (this.chicken.lives === 0 || this.seconds === 0) {
      ...
      var snd = new Audio("./audio/rooster.mp3");
      snd.play();

      setInterval(function () {
        that.ctx.font = "bold 200px Oswald";
        that.ctx.fillStyle = "blue";
        that.ctx.strokeStyle = "black";
        that.ctx.lineWidth = 20;
        that.ctx.strokeText("GAME OVER", that.dim_x/2 - 460, that.dim_y - 215);
        that.ctx.fillText("GAME OVER", that.dim_x/2 - 460, that.dim_y - 215);
      }, 50);
