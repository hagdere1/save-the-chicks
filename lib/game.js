(function () {
  ...
  Game.prototype.setEggsRemaining = function () {
    ...
    this.ctx.drawImage(bigEggIMG, 870, 15);

    this.ctx.font = "40px georgia";
    this.ctx.fillStyle = "blue";
    this.ctx.fillText(this.eggsCollected + "/" + this.totalEggs, 916, 50);
  };
  ...
  Game.prototype.drawTimer = function () {
    ...
    if (this.minutes > 0) {
      ...
      if (this.seconds > 9) {
        this.ctx.fillText(minutes + ":" + remainder, that.dim_x/2 - 15, 35);
      }
      else {
        this.ctx.fillText(minutes + ":0" + remainder, that.dim_x/2 - 15, 35);
      }
    }
    else {
      if (this.seconds > 9) {
        this.ctx.fillText(minutes + ":" + this.seconds, that.dim_x/2 - 15, 35);
      }
      else {
        this.ctx.fillText(minutes + ":0" + this.seconds, that.dim_x/2 - 15, 35);
      }
    }
  };
